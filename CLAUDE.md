...
- **grammar/raw/** — Data classes (`RawGrammar`, `RawRule`, `RawCapture`) and `GrammarReader` for parsing `.tmLanguage.json` files. Rules get unique sequential IDs assigned post-parse.
- **grammar/** — `TextMateGrammar` (placeholder for tokenizer logic)
- **theme/**, **registry/** — Placeholder directories for upcoming stages

### Implementation stages (from plan-poc.md)

Completed: Stage 0 (project setup), Stage 1 (Joni regex wrapper), Stage 2 (grammar parsing)
Pending: Stage 3 (rule compilation), Stage 4 (tokenizer), Stage 5 (theme engine), Stage 6 (Compose UI), Stage 7 (validation)
